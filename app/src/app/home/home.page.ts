...
export class HomePage {
  ...
  mostrarConsola() {
    ...
    if (this.user.usuario != "" && this.user.password != "") {
      this.mensaje = "Usuario en linea";
    } else {
      this.mensaje = "Usuario y/o contraseña no valido/s"
    }
  }
  ...
  confirm() {
    this.auth.register(this.user.usuario, this.user.password).then((res) => {
      if (res) {
        this.estado = "Este usuario ya existe";
      } else {
        this.mensaje = "Registro Completado";
        this.modal.dismiss(this.user.usuario, 'confirmar');
      }
    })
  }
